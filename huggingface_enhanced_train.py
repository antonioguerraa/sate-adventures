...
def download_wikitext_hf():
    """Download WikiText-2 dataset using HuggingFace datasets"""
    data_path = Path("data")
    data_path.mkdir(exist_ok=True)
    
    wikitext_path = data_path / "wikitext-2.txt"
    
    if not wikitext_path.exists():
        print("Downloading WikiText-2 dataset using HuggingFace...")
        try:
            # Load the wikitext dataset
            dataset = load_dataset("wikitext", "wikitext-2-raw-v1")
            
            # Combine all splits into one text file
            with open(wikitext_path, 'w', encoding='utf-8') as outfile:
                for split in ['train', 'validation', 'test']:
                    for item in dataset[split]:
                        outfile.write(item['text'] + '\n')
            
            print(f"Downloaded and processed to {wikitext_path}")
        except Exception as e:
            print(f"Error downloading WikiText: {e}")
            # Return a sample if download fails
            with open(wikitext_path, 'w', encoding='utf-8') as outfile:
                outfile.write("This is a sample WikiText content for testing purposes.\n")
                outfile.write("WikiText is a collection of over 100 million tokens extracted from the set of verified Good and Featured articles on Wikipedia.\n")
                outfile.write("The dataset is commonly used for language modeling benchmarks.\n")
            print(f"Created sample WikiText at {wikitext_path}")
...
def download_gutenberg():
    ...
    if not gutenberg_path.exists():
        # URLs for a few books from Project Gutenberg
        print("Downloading books from Project Gutenberg...")
        book_urls = [
            "https://www.gutenberg.org/files/1342/1342-0.txt",  # Pride and Prejudice
            "https://www.gutenberg.org/files/84/84-0.txt",      # Frankenstein
            "https://www.gutenberg.org/files/1080/1080-0.txt",  # A Tale of Two Cities
            ...
            "https://www.gutenberg.org/files/1184/1184-0.txt",  # Count of Monte Cristo
            "https://www.gutenberg.org/files/76/76-0.txt"       # Huckleberry Finn
        ]
        
        try:
            with open(gutenberg_path, 'w', encoding='utf-8') as outfile:
                for url in book_urls:
                    try:
                        book_path = data_path / f"temp_book_{url.split('/')[-1]}"
                        urllib.request.urlretrieve(url, book_path)
                        
                        with open(book_path, 'r', encoding='utf-8', errors='replace') as infile:
                            text = infile.read()
            ...
            print(f"Downloaded and processed to {gutenberg_path}")
        except Exception as e:
            print(f"Error in Gutenberg download: {e}")
            # Create a sample if downloading fails
            sample_text = """
...
def train():
    ...
    wikitext_path = download_wikitext_hf()
    print(f"WikiText data: {wikitext_path}")
    
    gutenberg_path = download_gutenberg()
    print(f"Gutenberg data: {gutenberg_path}")
    ...
    with open(gutenberg_path, 'r', encoding='utf-8', errors='replace') as f:
        all_text += f.read()
    print(f"After adding Gutenberg, text length: {len(all_text):,} characters")
    
    # Add WikiText (which is larger, so we'll sample a portion)
    with open(wikitext_path, 'r', encoding='utf-8', errors='replace') as f:
        wiki_text = f.read()
        # Use first 2 million characters to keep dataset balanced
        wiki_sample = wiki_text[:min(len(wiki_text), 2_000_000)]
        all_text += wiki_sample
    print(f"Final text length (with WikiText sample): {len(all_text):,} characters")
    ...
    # Save tokenizer
    tokenizer.save(data_dir / 'enhanced_tokenizer.json')
    ...
    # Create datasets and data loaders for efficient training
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        num_workers=0,  # Increase based on your CPU
    ...
    if lr_decay:
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=max_iters - warmup_iters, eta_min=learning_rate / 10
        )
    ...
    iter_num = 0
    progress_bar = tqdm(total=max_iters, desc="Training")
    ...
    try:
        while iter_num < max_iters:
            ...
            for x, y in train_loader:
                ...
                if lr_decay and iter_num < warmup_iters:
                ...
                if iter_num % eval_interval == 0 or iter_num == max_iters:
                    ...
                    val_loss = 0.0
                    with torch.no_grad():
                        for eval_step, (eval_x, eval_y) in enumerate(val_loader):
                            eval_x, eval_y = eval_x.to(device), eval_y.to(device)
                            _, loss = model(eval_x, eval_y)
                            val_loss += loss.item()
                            if eval_step >= eval_batches:
                                break
                    
                    val_loss /= min(eval_batches, len(val_loader))
                    val_losses.append(val_loss)
                    
                    print(f"\nIteration {iter_num}: train loss {train_losses[-1]:.4f}, val loss {val_loss:.4f}")
                    
                    # Generate sample text
                    context = torch.zeros((1, 1), dtype=torch.long, device=device)
                    context[0, 0] = tokenizer.encode('.')[0]  # Start with a period
                    sample = model.generate(context, max_new_tokens=200, temperature=0.8, top_k=50)[0].tolist()
                    ...
                    if val_loss < best_val_loss:
                        ...
                        torch.save(checkpoint, models_dir / 'enhanced_model_best.pt')
                    
                    # Resume training
                    model.train()
                
                # Break early if we've reached max iterations
    ...
    torch.save(checkpoint, models_dir / 'enhanced_model_final.pt')
    ...
    try:
        ...
        plt.subplot(1, 2, 2)
        x_vals = [i * eval_interval for i in range(len(val_losses))]
    ...
    for prompt in prompts:
        ...
        sample = model.generate(context, max_new_tokens=300, temperature=0.8, top_k=50)[0].tolist()
...
"""
    ...
    with open("enhanced_generate.py", "w") as f:
        f.write(inference_script)
    
    print("Created enhanced_generate.py script for inference.")
